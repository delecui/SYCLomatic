...
__device__ void foo_3() {
  ...
  int64_t m = 0;
  // CHECK: m = sycl::min(m, int64_t{static_cast<int64_t>((*pd3)[2])});
  // CHECK-NEXT: m = sycl::min(m, int64_t{static_cast<int64_t>((*pd3)[1])});
  // CHECK-NEXT: m = sycl::min(m, int64_t{static_cast<int64_t>((*pd3)[0])});
  // CHECK-NEXT: m = sycl::min(m, int64_t{static_cast<int64_t>(d3[2])});
  // CHECK-NEXT: m = sycl::min(m, int64_t{static_cast<int64_t>(d3[1])});
  // CHECK-NEXT: m = sycl::min(m, int64_t{static_cast<int64_t>(d3[0])});
...
void foo_4() {
  ...
  // CHECK: sycl::range<3> grid(1, std::min(maxGridDim, ceil_div(num_irows, int64_t{static_cast<int64_t>(threads[2])})), std::min(maxGridDim, num_orows));
